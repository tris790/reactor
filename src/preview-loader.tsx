...
function deserializeProps(props: any): any {
    function deserialize(value: any): any {
        if (value && typeof value === "object" && value.__type === "function") {
            const fn = (...args: any[]) =>
                console.log(`Mock function called: ${value.__name}`, args);
            return fn;
        }
...
function getTypeName(value: any): string {
    ...
    if (typeof value === "object" && value.__type === "function")
        return "function";
...
function PropsPanel({
    ...
    const [editingJson, setEditingJson] = React.useState<
    ...
    const renderPropValue = (key: string, value: any) => {
        ...
        if (enumMetadata && enumMetadata.values.length > 0) {
            return (
                <select
                    ...
                    value={value}
                    onChange={(e) => {
                        const enumValue = enumMetadata.values.find(
        ...
        if (type === "number") {
            return (
                <input
                    ...
                    value={value}
                    onChange={(e) =>
                        handleInputChange(key, e.target.value, "number")
                    }
                />
            );
        }
        ...
        if (type === "function") {
            return (
                <div className="prop-value">{value.__name || "function"}</div>
            );
        }
    ...
    const getDisplayTypeName = (key: string, value: any): string => {
        ...
        if (
            enumMetadata &&
            Array.isArray(enumMetadata.values) &&
            enumMetadata.values.length > 0
        ) {
...
async function render() {
    try {
        ...
        const propsResponse = await fetch(
            `/api/component-props?path=${encodeURIComponent(componentPath)}`
        );
        ...
        const translationModule = await import(
            /* @vite-ignore */ translationContextPath
        );
...
<div style="background: #ff4444; color: white; padding: 20px; border-radius: 8px; margin: 20px; font-family: system-ui, sans-serif;">
  ...
  <details style="margin-top:12px;">
    ...
    <pre style="white-space: pre-wrap; font-size:11px; background:#000; padding:10px; border-radius:4px; max-height:240px; overflow:auto;">${
